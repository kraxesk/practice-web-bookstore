...
<blockquote>
	...
	<input class="hi" style="float: right; margin: 2%;" type="button" name="cancel" value="Домой" onClick="window.location='index.php';" />
</blockquote>
</header>
<?php
session_start();
...
if(!isset($_SESSION['id'])){
	...
	echo 'Имя:<br><input type="text" name="name" placeholder="Полное имя">';
	...
	echo 'Номер мобильного телефона:<br><input type="text" name="contact" placeholder="+7">';
	echo '<span class="error" style="color: red; font-size: 0.8em;"><?php echo $contactErr;?></span><br><br>';

	echo '<label>Пол:</label><br>';
	echo '<input type="radio" name="gender" if (isset($gender) && $gender == "Male") echo "checked"; value="Male">Мужской';
	echo '<input type="radio" name="gender" if (isset($gender) && $gender == "Female") echo "checked"; value="Female">Женский';
	echo '<span class="error" style="color: red; font-size: 0.8em;"><?php echo $genderErr;?></span><br><br>';

	echo '<label>Адрес:</label><br>';
	   echo '<textarea name="address" cols="30" rows="5" placeholder="Адрес"></textarea>';
